...
@section('content')	
{{ Breadcrumbs::render('reporte_company') }}
               ...
               $(tableName).DataTable().clear();
                ...
                tbl=  $(tableName).dataTable({
                    ...
                    // scrollX : true,
                      ...
                      ajax:'{!! route('report.getiequipmentByCompany.year')!!}?year='+yearr,
                      "columns" : [                     
                      ...
                      "lengthMenu": [
                       ...
                       buttons: [ 'excel', 'pdf', 'print'], ///'<"clear">lftiprB',
                       dom: 
                        "<'row'<'col-sm-6'l><'col-sm-6 text-right'f>><'row'<'col-sm-12'tr>><'row'<'col-sm-6'i><'col-sm-6'p>>B",
                      responsive: true,
                      language: {
                        search: "_INPUT_",
...
function datatable_filter(from,to){
    ...
    var fdate = currentd.getFullYear()-1  + "-" + (currentd .getMonth() + 1) + "-" + currentd.getDate()
      ...
      var data, tableName= '#table_company',     str, strf
                ...
                tbl =  $(tableName).dataTable({
                     ...
                     scrollX : true,
                      ...
                      ajax:'{!! route('report.getiequipmentByCompany.filter')!!}?from='+froms+'&to='+tos,
                      "columns" :  [                     
                        { data: 'company_name', name: 'company_name' ,"visible": false },
...
$("#searchsub").click(function(){
  $.LoadingOverlay("show");
   ...
   total_year.api().ajax.url('{!! route('report.getiequipmentByCompany.year')!!}?year='+yearr);
...
$("#searchf").click(function(){
  $.LoadingOverlay("show");
   ...
   total_year.api().ajax.url('{!! route('report.getiequipmentByCompany.filter')!!}?from='+froms+'&to='+tos);
   total_year.api().column(0).search( opt ).draw();
     $.LoadingOverlay("hide");
});
