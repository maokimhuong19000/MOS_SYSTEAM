@extends('layout.front') @section('content') @include('layout.partical.headuser') <form id="form_isub" action="{{route('isubstance.save')}}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    <div class="container">
        <div class="col-sm-12 col-xs-12 padding_bottom padding-top-lg-extra bg_child ">
            ...
            <div class="col-md-12 ">
                <div class="flash-message"> @foreach (['error', 'warning', 'success', 'info'] as $key) @if(Session::has($key)) <p class="alert alert-danger">{{ Session::get($key) }}
                    </p> @endif @endforeach </div> @if ($errors->any()) <div class="alert alert-danger">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <span class="star_require text-danger">*</span>
                                <select name="from_country_id" class="form-control"> @foreach($countries as $country) <option value="{{$country->id}}" {{ (old('from_country_id') == $country->id ? "selected":"" )}}>{{$country->countries_name}}</option> @endforeach </select>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="manufacture_country_id" class="form-control"> @foreach($countries as $country) <option value="{{$country->id}}" {{ (old('manufacture_country_id') == $country->id ? "selected":"" )}}>{{$country->countries_name}}</option> @endforeach </select>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <span class="star_require text-danger">*</span>
                                <select id="place_export" name="place_export" class="form-control"> @foreach($exportPort as $port) <option value="{{$port->port_code}}" {{ (old('place_export') == $port->port_code? "selected":"" )}}>
                                        {{$port->port_name}}
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="tcountry" class="form-control tcountry" name="tcountry" disabled="disabled"> @foreach($countries as $country) <option value="{{$country->id}}">{{$country->countries_name}}</option> @endforeach </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <span> {{trans('front_isubstance.transit_port')}}&nbsp;&nbsp;:</span>
                                <select id="des_port" class="form-control des_port" name="des_port" disabled="disabled"> @foreach($exportPort as $port) <option value="{{$port->port_code}}">
                                        {{$port->port_name}}
                                    </option> @endforeach </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <span class="star_require text-danger">*</span>
                                <select name="place_import" id="place_import" class="form-control"> @foreach($entry as $entries) <option value="{{$entries->id}}" {{ (old('place_import')== $entries->id ? "selected":"")}}>
                                        {{$entries->port_name}}
                                    </option> @endforeach </select>
                            </div>
                        </div>
                ...
                <div class="panel-body ">
                    ...
                    <div class="col-sm-12 col-xs-12 mtable">
                        <table class="table table-responsive" id="table_isubstance">
                            ...
                            <tbody> @if (old('total')) @foreach(old('total') as $index => $value) <tr id="row_{{$index}}">
                                    <td class='order'>{{$index+1}}</td>
                                    <td><input type='hidden' name='material_id[]' id='material_id.row_{{$index+1}}' value="{{old('material_id')[$index]}}">{{old('sub')[$index]}}
                                    ...
                                    <td><input type="hidden" name="store_type[]" id="store.row_{{$index+1}}" value="{{old('store_type')[$index]}}">{{old('store_type')[$index]}}Kg/cly</td>
                                    <td><input type="hidden" name="number[]" id="number.row_{{$index+1}}" value="{{old('number')[$index]}}">
                                        <input type="hidden" name="uom[]" id="uom.row_{{$index+1}}" value="{{old('uom')[$index]}}">
                                        {{old('number')[$index]}}{{old('uom')[$index]}}
                                    </td>
                                    <td><input type="hidden" name="total[]" id="total.row_{{$index+1}}" value="{{$value}}">{{$value}}</td>
                                    <td><input type="hidden" name="gross[]" id="gross.row_{{$index+1}}" value="{{old('gross')[$index]}}">{{old('gross')[$index]}}</td>
                                    <td><input type="hidden" name="invoicevalue[]" id="invoicevalue.row_{{$index+1}}" value="{{old('invoicevalue')[$index]}}">{{old('invoicevalue')[$index]}}</td>
                                    <td><input type="hidden" name="quality[]" id="qu.row_{{$index+1}}" value="{{old('quality')[$index]}}">{{old('quality')[$index]}}</td>
                                ...
                                <div class="modal-body">
                                    <form id="form-modal">
                                        <div class="form-group">
                                            ...
                                            <span class="star_require text-danger">*</span>
                                            <select name="" class="form-control substance" id="substance"> @foreach($Material as $material) <option value="{{$material->material_id}}">{{$material->substance}}</option> @endforeach </select>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-xs-6">
                                                <div class="form-group">
                                                    ...
                                                    <select name="" class="form-control uom" id="uom" autocomplete="off"> @foreach($uom as $u) <option value="{{$u->code}}">{{$u->description}}</option> @endforeach </select>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group">
    ...
    $(document).ready(function() {
        ...
        $('body').on('change', '[name="from_country_id"]', function() {
            $.ajax({
                ...
                front.exportport ')!!}',
                data: {
                    country: $('select[name="from_country_id"] option:selected').val(),
                },
                beforeSend: function() {
                    $.LoadingOverlay("show");
                },
        ...
        $("#add_sub").click(function() {
            ...
            // console.log(new_id);
            var markup = "<tr id='" + new_id + "'>" + "<td class='order'>" + (Number(id) + 1) + "</td>" + "<td>" + "<input type='hidden' name='material_id[]' id='material_id." + new_id + "' value='" + material_val + "'>" + $("#substance option:selected").text() + '<input type="text" class="com_pany" id="com_pany" name="com_pany[]" disabled>' + "</td>" + '<input type="hidden" class="subc"  name="sub[]" value="' + $("#substance option:selected").text() + '">' + '<input type="hidden" class="subcom"  name="com[]" value="">' + "</td>" + "<td>" + "<input type='hidden' name='store_type[]' id='store." + new_id + "' value='" + other + "'>" + other + "Kg/cly</td>" + "<td>" + "<input type='hidden' name='uom[]' id='uom." + new_id + "' value='" + uom + "'>" + "<input type='hidden' name='number[]' id='number." + new_id + "' value='" + number + "'>" + number + " " + uom + "</td>" + "<td>" + "<input type='hidden' name='total[]' id='total." + new_id + "' value='" + total + "'>" + total + "</td>" + "<td>" + "<input type='hidden' name='gross[]' id='gross." + new_id + "' value='" + gross + "'>" + gross + "</td>" + "<td>" + "<input type='hidden' name='invoicevalue[]' id='invoicevalue." + new_id + "' value='" + invoicevalue + "'>" + invoicevalue + "</td>" + "<td>" + "<input type='hidden' name='quality[]' id='qu." + new_id + "' value='" + qu + "'>" + qu + "</td>" + "<td>" + "<input type='checkbox' name='record'>" + "</td>" + "</tr>";
        ...
        $('body').on('change', '[name="tcountry"]', function() {
            $.ajax({
                ...
                front.exportport ')!!}',
                data: {
                    country: $('select[name="tcountry"] option:selected').val(),
                },
                beforeSend: function() {
                    $.LoadingOverlay("show");
                },
