@extends('layout.front') @section('content') @include('layout.partical.headuser') <form id="equipment" action="{{route('equipmentrequest.store')}}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    <div class="container">
                    ...
                    <div class="col-md-12 ">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <span class="star_require text-danger">*</span>
                                <select name="country_id" class="form-control"> @foreach($countries as $country) <option value="{{$country->id}}" {{ (old('from_country_id') == $country->id ? "selected":"" )}}>{{$country->countries_name}}</option> @endforeach </select>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <span class="star_require text-danger">*</span>
                                <select id="place_export" name="place_export" class="form-control"> @foreach($exportPort as $port) <option value="{{$port->port_code}}" {{ (old('place_export') == $port->port_code? "selected":"" )}}>{{$port->port_name}}</option> @endforeach </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                ...
                <div class="panel-body ">
                    ...
                    <div class="col-sm-12 col-xs-12 mtable">
                        <table class="table table-responsive" id="table_isubstance">
                            ...
                            <tbody> @if (old('amount')) @foreach(old('amount') as $index => $value) <tr id="row_{{$index}}">
                                    <td class='order'>{{$index+1}}</td>
                                    <td><input type='hidden' name='equitment_id[]' id='material_id.row_{{$index+1}}' value="{{old('equitment_id')[$index]}}">{{old('sub')[$index]}}
                                    </td>
                                    ...
                                    <td><input type="hidden" name="des[]" id="store.row_{{$index+1}}" value="{{old('des')[$index]}}">{{old('des')[$index]}} <br />
                                        <input type="hidden" name="substance[]" id="total.row_{{$index+1}}" value="{{old('substance')[$index]}}">Use Substance:{{old('substance')[$index]}}
                                    </td>
                                    <td><input type="hidden" name="capacity[]" id="number.row_{{$index+1}}" value="{{old('capacity')[$index]}}">{{old('capvalue')[$index]}} {{old('capacity')[$index]}}
                                        <input type="hidden" name="capvalue[]" id="number.row_{{$index+1}}" value="{{old('capvalue')[$index]}}">
                                    </td>
                                    <!-- addnew -->
                                    <td>
                                    ...
                                    <td>
                                        <input type="hidden" name="invoicevalue[]" id="invoicevalue.row_{{$index+1}}" value="{{old('invoicevalue')[$index]}}">{{old('invoicevalue')[$index]}}
                                    </td>
                                    <!-- addend -->
                                    <td><input type="hidden" name="substance[]" id="total.row_{{$index+1}}" value="{{old('substance')[$index]}}">{{old('substance')[$index]}}</td>
                                        ...
                                        <div class="row">
                                            <div class="col-xs-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="netweight" class="form-control net" id="netweight">
                                                </div>
                                            </div>
                                            <div class="col-xs-6">
                                                <div class="form-group">
                                                    <span>{{trans('iequipment.gross')}}</span><span class="star_require text-danger">*</span>
                                                    <input type="text" name="gross" class="form-control gross" id="grossweight">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group">
    ...
    $(document).ready(function() {
        ...
        $('body').on('change', '[name="country_id"]', function() {
            $.ajax({
                ...
                front.exportport ')!!}',
                data: {
                    country: $('select[name="country_id"] option:selected').val(),
                },
                beforeSend: function() {
                    $.LoadingOverlay("show");
                },
        ...
        $('#add_sub').click(function() {
            ...
            //}
            //else{
            var markup = "<tr id='" + new_id + "'>" + "<td>" + (Number(id) + 1) + "</td>" + "<td>" + "<input type='hidden' name='equitment_id[]' value='" + equitment + "'>" + $('#equitment option:selected').text() + "</td>" + "<td>" + "<input type='hidden' name='uom[]' id='uom." + new_id + "' value='" + uom + "'>" + "<input type='hidden' name='amount[]' value='" + amount + "'>" + amount + " " + uom + "<input type='hidden' class='subc'  name='sub[]' value='" + $('#equitment option:selected').text() + "' >" + "</td>" + "<td>" + "<input type='hidden' name='des[]' value='" + des + "'>" + des + "</td>" + "<td>" + "<input type='hidden' name='capacity[]' value='" + capacity + "'>" + capvalue + capacity + "<input type='hidden' name='capvalue[]' value='" + capvalue + "'>" + "</td>" + "<td>" + "<input type='hidden' name='net[]' id='net." + new_id + "' value='" + net + "'>NET:" + net + "</br>" + "<input type='hidden' name='gross[]' id='gross." + new_id + "' value='" + gross + "'>GROSS:" + gross + "</td>" + "<td>" + "<input type='hidden' name='invoicevalue[]' id='invoicevalue." + new_id + "' value='" + invoicevalue + "'>" + invoicevalue + "USD" + "</td>" + "<td>" + "<input type='hidden' name='substance[]' value='" + substance + "'>" + substance + "</td>" + "<td>" + "<input type='hidden' name='quality[]' id='qu." + new_id + "' value='" + qu + "'>" + qu + "</td>" + "<td>" + "<input type='checkbox' name='record'>" + "</td>" + "</tr>";
