...
class HomeController extends Controller
{
    ...
    public function resetpwd(Request $request)
    {
        $validatedData = $request->validate(
            ...
            [
                "oldpassword.required" =>
                    "សូមបញ្ចូលពាក្យសំងាត់ចាស់/Please input old password",
                "password.required" =>
                    "សូមបញ្ចូលពាក្យសំងាត់ថ្មី/Please input new password",
                "password.min" =>
                    "ពាក្យសំងាត់ត្រូវការយ៉ាងតិច ៦តួ/ Password require at less 6 digit",
                "password.confirmed" =>
                    "កាបញ្ចូលពាក្យសំងាត់លើកទី២មិនត្រឹមត្រូវ/ Comfirm Password dose not matched",
            ],
        );
        ...
        if (!Hash::check($request->input("oldpassword"), $user->password)) {
            return redirect()
                ...
                ->with(
                    "errors",
                    "កូដសំងាត់ចាស់មិនត្រឹមត្រូវ/ Current Password dose not matched !",
                )
                ->withInput();
        } else {
            ...
            return redirect()
                ...
                ->with(
                    "success",
                    "កូដសំងាត់ប្តូរដោយជោគជ័យ/ Password changed successfully !",
                );
        }
    }
    ...
    public function reporte()
    {
        ...
        $from = @$_GET["from"]
            ? date("Y-m-d H:i:s", strtotime($_GET["from"]))
            : date("Y-m-d");
        $to = @$_GET["to"]
            ? date("Y-m-d H:i:s", strtotime($_GET["to"]))
            : date("Y-m-d");
        ...
        sum(case when equipmentrequests.created_at >=  '" .
            $from .
            "' AND equipmentrequests.created_at <=  '" .
            $to .
            "' AND equipmentrequests.import_status > 1   then equipmentrequestdetails.amount else 0 end) as total 
        FROM   `equipment`
        ...
        INNER JOIN equipmentrequests   ON `equipmentrequests`.`id` = `equipmentrequestdetails`.`equipmentrequest_id`    
          WHERE `status` = 1 AND equipmentrequests.customer_id =" .
            Auth::id() .
            "     Group by product_name , equipment.id ,   `taxcode`, capvalue*capvalue_data ,equipmentrequestdetails.substance,
        ...
        , sum(case when year(equipmentrequests.created_at)= " .
            $year .
            " AND equipmentrequests.import_status > 1   then equipmentrequestdetails.amount else 0 end) as total 
        FROM   `equipment`
        INNER JOIN  `equipmentrequestdetails`   on  equipmentrequestdetails.equipment_id = equipment.id  
    ...
    public function reportm()
    {
        ...
        , sum(case when year(materialrequests.created_at)= " .
            $year .
            " AND materialrequests.import_status > 1   then materialrequestdetails.quantity else 0 end) as total 
        FROM   `materials`
        ...
        INNER JOIN materialrequests   ON `materialrequests`.`id` = `materialrequestdetails`.`materialrequest_id`    
          WHERE `status` = 1 AND materialrequests.customer_id = " .
            Auth::id() .
            "    Group by year(materialrequests.created_at), substance , materials.id , `com_name`, `chemical`, `substance`, `code`, `taxcode`";

        $sql1 =
            " SELECT 
        substance , materials.id ,  `com_name`, `chemical`, `substance`, `code`, `taxcode`
        , sum(case when materialrequests.created_at >=  '" .
            $from .
            "' AND materialrequests.created_at <=  '" .
            $to .
            "' AND materialrequests.import_status > 1   then materialrequestdetails.quantity else 0 end) as total 
        FROM   `materials`
        INNER JOIN  `materialrequestdetails`   on  materialrequestdetails.material_id = materials.id  
        INNER JOIN materialrequests   ON `materialrequests`.`id` = `materialrequestdetails`.`materialrequest_id`    
          WHERE `status` = 1 AND materialrequests.customer_id = " .
            Auth::id() .
            "    Group by  substance , materials.id , `com_name`, `chemical`, `substance`, `code`, `taxcode`";
        $Aquota = @$_GET["from"]
            ? DB::select(DB::raw($sql1))
            : DB::select(DB::raw($sql));
        return datatables($Aquota)->toJson();
    ...
    public function reportq()
    {
      ...
      Left JOIN 
        ...
        WHERE `materialrequests`.`import_status` > 1 AND materialrequests.customer_id =  " .
            Auth::id() .
            "
    ...
    public function uprofile(Request $request)
    {
        ...
        $validator = Validator::make(
            $request->all(),
            $rule,
            Customer::$messages_front_profile,
        );
        if ($validator->fails()) {
            ...
            if ($request->hasFile("tin_path")) {
                ...
                $file_tin->move(
                    $destinationPath,
                    $file_tin->getClientOriginalName(),
                );
                $tin =
                    $destinationPath . "/" . $file_tin->getClientOriginalName();
            }
            ...
            $all_data = [
                ...
                "tin_date" => date(
                    "Y-m-d H:i:s",
                    strtotime($request->input("tin_date")),
                ),
                "company_id" => $request->input("company_id"),
                "company_id_date" => date(
                    "Y-m-d H:i:s",
                    strtotime($request->input("company_id_date")),
                ),
                ...
                "patent_date" => date(
                    "Y-m-d H:i:s",
                    strtotime($request->input("patent_date")),
                ),
            ];
            //return view('profile', compact('Customer'));
    ...
    public function ucontact(Request $request)
    {
        ...
        $validator = Validator::make(
            $request->all(),
            $rule,
            Cominfo::$messages_front_contact,
        );
        if ($validator->fails()) {
            return \Redirect::back()
                ->withErrors($validator)
            ...
            if ($request->hasFile("id_card")) {
                $file_tin = $request->file("id_card");
                $destinationPath =
                    "uploads/cominfo/id/" . $Customer->Cominfo->id;
                $file_tin->move(
                    $destinationPath,
                    $file_tin->getClientOriginalName(),
                );
            ...
            if ($request->hasFile("photo")) {
                ...
                $destinationPath =
                    "uploads/cominfo/photo/" . $Customer->Cominfo->id;
                $file_tin->move(
                    $destinationPath,
                    $file_tin->getClientOriginalName(),
                );
            ...
            if ($request->hasFile("authorize")) {
                ...
                $destinationPath =
                    "uploads/cominfo/authorize/" . $Customer->Cominfo->id;
                $file_tin->move(
                    $destinationPath,
                    $file_tin->getClientOriginalName(),
                );
    ...
    public function isubstance()
    {
        ...
        $invoicevalue=$invoice_value::all();
    ...
    public function isubdeclare(Request $request, $id)
    {
        ...
        if (isset($image1)) {
            ...
            $path =
                "uploads/file_import_department/" .
                Auth::id() .
                "/" .
                date("Y-m-d", strtotime($request->import_date));
            if (!file_exists($path)) {
        ...
        foreach ($users as $user) {
            if ($user->hasAllPermissions(16)) {
                Mail::send(
                    "emails.custom_alert",
                    ["requestdetail" => $material, "user" => $user],
                    function ($m) use ($user) {
                        $m->from(
                            "developer@cems10.com",
                            "MOE Mail Auto System",
                        );

                        $m->to($user->email, $user->name)->subject(
                            "User Upload Custom Declaration!",
                        );
                    },
                );
            }
        ...
        return redirect()
            ->route("isubstance.isubstance_show", $id)
            ->with(
                "success",
                " បញ្ចូនប្រតិវេទយ៍គយមរួចរាច់ / Upload Custom Declaration success",
            );
    }
    ...
    public function isubStore(Request $request)
    {
        ...
        $validatedData = $request->validate(
            ...
            [
                "manufacture_name.required" =>
                    "សូមបញ្ចូលឈ្មោះក្រុមហ៊ុនដែលនាំចេញ/Please Input Export Company",

                "address.required" =>
                    "សូមបញ្ចូលអាសយដ្ឋានក្រុមហ៊ុននាំចេញ/Please Export Company Address ",
                "import_port.required" =>
                    "សូមបញ្ចូលឈ្មោះរោងចក្រផលិត/Please Manufacturer Company Name",
                "address.*.required" =>
                    "សូមបញ្ចូលអាសយដ្ឋានក្រុមហ៊ុននាំចេញ/Please Export Company Address ",
                "import_port.*.required" =>
                    "សូមបញ្ចូលឈ្មោះរោងចក្រផលិត/Please Manufacturer Company Name",

                "file_shipping.required" =>
                    "សូមភ្ជាប់មកជាមួយឯកសារដឹកជញ្ជូន/Please Attach Shipping Document",
                "file_custom_declar.required" =>
                    "សូមភ្ជាប់មកជាមួយឯកសារទំនិញ/Please Attach Packing list",
                "file_invoice.required" =>
                    "សូមភ្ជាប់មកជាមួយវិកយបត្រទីញទំនិញ/Please Attach Invoice",

                "billnumber.*.required" =>
                    "សូមបញ្ចូលលេខវិក័យប័ត្រ/Please Input Invoice No",
                "invoice_value_other_currency.*.required" =>
                    "សូមបញ្ចូលប្រាក់សរុបក្នុងវិក័យប័ត្រ/ Please Input Invoice Total Price",
                "billnumber.required" =>
                    "សូមបញ្ចូលលេខវិក័យប័ត្រ/Please Input Invoice No",
                "invoice_value_other_currency.required" =>
                    "សូមបញ្ចូលប្រាក់សរុបក្នុងវិក័យប័ត្រ/ Please Input Invoice Total Price",
                ...
                "material_id.*.required" =>
                    "សូមបញ្ចូលសារធាតុដែលត្រូវនាំចូល/Please select Import Substance",
                "store_type.*.required" =>
                    "សូមជ្រើសរើសប្រភេទ ប្រដាប់សម្រាប់ដាក់សារធាតុ/Please select Container or Cylinder",
                "number.*.required" =>
                    "សូមបញ្ចូលចំនួន/Please Input Number of Cylinder",
        ...
        foreach ($request->input("number") as $index => $value) {
            if ($value != null) {
                ...
                $result = $this->check_quota(
                    $request->material_id[$index],
                    Auth::id(),
                    $request->total[$index],
                );
        ...
        $materialrequest->manufacture_country_id =
            $request->manufacture_country_id;
        $materialrequest->from_country_id = $request->from_country_id;
        $materialrequest->import_port = $request->import_port;
        $materialrequest->import_date = \App\Helpers\AppHelper::instance()->format_insert_date(
            $request->import_date,
        );
        ...
        $materialrequest->aircon_service_option =
            $request->purpose == 2 ? 1 : 0;
        ...
        $materialrequest->other_option_description =
            $request->other_option_description;
        $materialrequest->self_usage_percent = $request->self_usage_percent
            ? $request->self_usage_percent
            : 0;
        $materialrequest->other_usage_percent = $request->other_usage_percent
            ? $request->other_usage_percent
            : 0;
        $materialrequest->other_info = $request->other_info;
        //$materialrequest->quality=$request->quality;
        $materialrequest->place_import = $request->place_import
            ? $request->place_import
            : "";
        $materialrequest->place_export = $request->place_export
            ? $request->place_export
            : "";

        $materialrequest->transport = $request->transport
            ? $request->transport
            : 1;
        $materialrequest->transit = $request->transit ? $request->transit : 0;
        $materialrequest->tcountry = $request->tcountry
            ? $request->tcountry
            : 0;
        $materialrequest->des_port = $request->des_port
            ? $request->des_port
            : "";
        $materialrequest->incoterm = $request->incoterm
            ? $request->incoterm
            : "";
        $materialrequest->billnumber = $request->billnumber
            ? $request->billnumber
            : "";
        $materialrequest->billdate = \App\Helpers\AppHelper::instance()->format_insert_date(
            $request->billdate,
        );
        $materialrequest->currency = $request->currency
            ? $request->currency
            : "USD";
        $materialrequest->exchange_rate = $request->exchange_rate
            ? $request->exchange_rate
            : 4100;
        $materialrequest->invoice_value_other_currency = $request->invoice_value_other_currency
      ...
      //  dd($materialrequest);
        ...
        foreach ($users as $user) {
            if ($user->hasAnyPermission(13)) {
                Mail::send(
                    "emails.req",
                    ["requestdetail" => $materialrequest, "user" => $user],
                    function ($m) use ($user) {
                        $m->from(
                            "developer@cems10.com",
                            "MOE Mail Auto System",
        ...
        return redirect()
            ...
            ->with(
                "success",
                'ការស្នើសុំនាំចូលរបស់លោកអ្នក បានរួចរាល់ សូមរង់ចាំ ការឆ្លើយតបពីក្រសួងបរិស្ថាន
/ Your Request sent successful, Please wait for approval from Ministry of Environment. ',
    ...
    public function esubstance($id)
    {
        $Material = Aquota::join(
            ...
            ->join("materials", "materials.id", "=", "aquotas.material_id")
            ->get();
        $isubdetail = Materialrequest::with([
            "Materialrequestdetails.Material",
            "Country",
            "Port_Entries",
            "mCountry",
            "Iladings",
        ...
        $Equipmentrequest = Equipmentrequest::with([
            "Equipmentrequestdetail.Equipment",
            "Port_Entries",
            "Country",
            "mCountry",
            "Eladings",
    ...
    public function usubstance(
        Request $request,
        $id,
        Materialrequest $materialrequest_id
    ) {
        // check substance is null
        $user = DB::table("materialrequestdetails")
            ->where("materialrequest_id", $id)
            ->first();
        ...
        if (!$user) {
            $validatedData = $request->validate(
                ...
                [
                    ...
                    "number.*.required" =>
                        "សូមបញ្ចូលចំនួន/Please Input Number of Cylinder",
                    "total.*.required" =>
                        "សូមបញ្ចូលបរិមាណ/Please Input quantity",
            ...
            if (!$validatedData) {
                ...
                $all_data = [
                    ...
                    "manufacture_country_id" => $request->input(
...
//                  $materialrequest->import_date=\App\Helpers\AppHelper::instance()->format_insert_date($request->import_date);
                    ...
                    "aircon_service_option" =>
                        $request->input("purpose") == 2 ? 1 : 0,
                    "other_option" => $request->input("purpose") == 3 ? 1 : 0,
                    "other_option_description" => $request->input(
                        "other_option_description",
                    ),
                    "self_usage_percent" => $request->input(
                        "self_usage_percent",
                    ),
                    "other_usage_percent" => $request->input(
                        "other_usage_percent",
                    ),
                ...
                foreach ($request->input("number") as $index => $value) {
                    if ($value != null) {
                        ...
                        $all_data = [
                            ...
                            "invoice_value_other_currency" => $request
                                ->invoice_value_other_currency[$index]
                                ? $request->invoice_value_other_currency[$index]
                                : 0,
                            "grossweight" => $request->gross[$index]
                                ? $request->gross[$index]
                                : 0,
                            ...
                            if ($value != null) {
                                ...
                                $all_data = [
                                    "material_id" =>
                                        $request->material_id[$index],
                                    "store_type" =>
                                        $request->store_type[$index],
                                    "number" => $value,
                                    "invoicevalue" =>
                                        $request->invoicevalue[$index],
                                    "quantity" => $request->total[$index],
                                    "quality" => $request->quality[$index],
                                ];
                                $isubdetail
        ...
        if ($request->delete) {
            if (
                !$request->input("file_fs") &&
                !$request->hasFile("file_shipping")
            ) {
                $validatedData = $request->validate(
                    [
                        "file_shipping" => "required|array",
            ...
            if (
                !$request->input("file_fc") &&
                !$request->hasFile("file_custom_declar")
            ) {
                $validatedData = $request->validate(
                    [
                        "file_custom_declar" => "required|array",
            ...
            if (
                !$request->input("file_fi") &&
                !$request->hasFile("file_invoice")
            ) {
                $validatedData = $request->validate(
                    [
                        "file_invoice" => "required|array",
        ...
        return redirect()
            ...
            ->with(
                "success",
                "ការកែប្រែរបស់លោកអ្នកបានជោគជ័យ/Update Successful!",
            )
            ->withInput();
    }
    ...
    public function iequdeclare(Request $request, $id)
    {
        ...
        return redirect()
            ->route("front.showdetail_equipmentrequest", $id)
            ->with(
                "success",
                " បញ្ចូនប្រតិវេទយ៍គយមរួចរាច់ / Upload Custom Declaration success",
            );
    ...
    {
        $mrequest = Materialrequest::join(
            "customers",
            "materialrequests.customer_id",
            "=",
            "customers.id",
        )
        ...
        if ($wordCount > 0) {
            $result["status"] = 1;
            $result["message"] =
                "កាស្នើសុំចាស់ មិនទាន់បញ្ចប់ សូម ដាក់បញ្ចូល ប្រតិវេទ​ន៍​គយ​ ក្នុងការស្នើសុំចាស់/ Last Request not yet finished . Please upload Custom Declaration to finish it ";
        } else {
            $result["status"] = 0;
            $result["message"] = "";
        }
    ...
    {
        $sql =
            "SELECT Quota.`year` , Quota.amount , substance , company_name , Quota.customer_id , Quota.material_id , Imported.total_use
         ...
         FROM 
            ...
            where `materials`.`id` =  " .
            $material_id .
            " AND comquotas.customer_id = " .
            $customer_id .
            " AND aquotas.year = YEAR(CURDATE())     ) 
            AS Quota 
            ...
            where materialrequestdetails.material_id = " .
            $material_id .
            " and year(import_date)= YEAR(CURDATE()) AND `materialrequests`.`import_status` > 1 AND materialrequests.customer_id = " .
            $customer_id .
            "
        ...
        if ($Aquota) {
            ...
            $total_use = is_null($Aquota[0]->total_use)
                ? 0
                : $Aquota[0]->total_use;
    ...
    public function isubstance_detail($id)
    {
        ...
        $mcon_get = Country::where(
            "countries_name",
            "=",
            $isubdetail->manufacture_country_id,
        )->first();
        $con_get = Country::where(
            "countries_name",
            "=",
            $isubdetail->from_country_id,
        )->first();
        return view(
            "isubstance_show_detail",
            compact(
                "isubdetail",
                "countries",
                "mcon_get",
    ...
    {
        $mrequest = Equipmentrequest::join(
            "customers",
            "equipmentrequests.customer_id",
            "=",
            "customers.id",
        )
            ->where("import_status", 2)
        ...
        if ($wordCount > 0) {
            $result["status"] = 1;
            $result["message"] =
                "កាស្នើសុំចាស់ មិនទាន់បញ្ចប់ សូម ដាក់បញ្ចូល ប្រតិវេទ​ន៍​គយ​ ក្នុងការស្នើសុំចាស់/ Previous Request is not yet finished . Please upload Custom Declaration to finish it ";
        } else {
            $result["status"] = 0;
            $result["message"] = "";
        }
    ...
    public function uequipment(Request $request, $id)
    {
        ...
        $user = DB::table("equipmentrequestdetails")
            ->where("equipmentrequest_id", $id)
            ->first();
        if (!$user) {
            $validatedData = $request->validate(
                [
                ...
                [
                    "manufacture_name.required" =>
                        "សូមបញ្ចូលឈ្មោះក្រុមហ៊ុនដែលនាំចេញ/Please Input Export Company",
                    "equipment_id.*.required" =>
                        "សូមបញ្ចូលប្រភេទបរិក្ខាដែលត្រូវនាំចូល/Please select Import Equipment",
                    "capacity.*.required" =>
                        "សូមបញ្ចូលCapacity/Please input Capacity",
                    "amount.*.required" =>
                        "សូមបញ្ចូលចំនួន/Please Input Number of unit",
            ...
            if (!$validatedData) {
                ...
                $equitmentrequest->manufacture_name =
                    $request->manufacture_name;
                $equitmentrequest->country_id = $request->country_id;
                $equitmentrequest->manufacture_country_id =
                    $request->manufacture_country_id;
                $equitmentrequest->import_port = $request->import_port;
                $equitmentrequest->import_date = \App\Helpers\AppHelper::instance()->format_insert_date(
                    $request->import_date,
                );
                ...
                $equitmentrequest->manufacture_option =
                    $request->purpose == 1 ? 1 : 0;

                $equitmentrequest->aircon_service_option =
                    $request->purpose == 2 ? 1 : 0;
                $equitmentrequest->billdate = $request->billdate;
                $equitmentrequest->billnumber = $request->billnumber;
                $equitmentrequest->invoice_value_other_currency =
                    $request->invoice_value_other_currency;
                $equitmentrequest->currency = $request->currency;
                $equitmentrequest->incoterm = $request->incoterm;

                $equitmentrequest->other_option =
                    $request->purpose == 3 ? 1 : 0;
                $equitmentrequest->other_option_description =
                    $request->other_option_description;
                ...
                foreach ($request->input("amount") as $index => $value) {
                    if ($value != null) {
                        ...
                        $all_data = [
                            ...
                            "invoice_value" =>
                                $request->invoice_value[$index] ?? 0,
                            "uom" => $request->uom[$index]
                                ? $request->uom[$index]
                                : 0,
                            "capvalue" => $request->capvalue[$index]
                                ? $request->capvalue[$index]
                                : 0,
                    ...
                    $validatedData = $request->validate(
                        ...
                        [
                            ...
                            "amount.*.required" =>
                                "សូមបញ្ចូលចំនួន/Please Input Number of unit",
                            "des.*.required" =>
                                "សូមបញ្ចូលឈ្មោះ/Please Input Name",
        ...
        if ($request->hasFile("file_custom_declar")) {
            ...
            foreach ($request->file("file_custom_declar") as $index => $image) {
                if (isset($image)) {
                    ...
                    if (
                        !$equitmentrequest->Epackinglists()->create($all_data)
                    ) {
                        return \Redirect::back()
                            ->with("error", "Some thing went wrong!!")
                            ->withInput();
        ...
        return redirect()
            ...
            ->with(
                "success",
                "ការកែប្រែរបស់លោកអ្នកបានជោគជ័យ / Your Are Update successful!",
            );
    }
    ...
    public function store(Request $request)
    {
        ...
        $equitmentrequest->manufacture_country_id =
            $request->manufacture_country_id;
        $equitmentrequest->import_port = $request->import_port;
        $equitmentrequest->transport = $request->transport
            ? $request->transport
            : 1;
        $equitmentrequest->incoterm = $request->incoterm
            ? $request->incoterm
        ...
        $equitmentrequest->billnumber = $request->billnumber
            ? $request->billnumber
            : "";
        $equitmentrequest->billdate = \App\Helpers\AppHelper::instance()->format_insert_date(
            $request->billdate,
        );
        $equitmentrequest->currency = $request->currency
            ? $request->currency
            : "USD";
        $equitmentrequest->exchange_rate = $request->exchange_rate
            ? $request->exchange_rate
            : 4100;
            ...
            if ($user->hasAnyPermission(24)) {
                Mail::send(
                    ...
                    function ($m) use ($user) {
                        ...
                        $m->to($user->email, $user->name)->subject(
                            "User Request Permit For Import Equipment!",
                        );
                    },
                );
            }
        ...
        return redirect()
            ...
            ->with(
                "success",
                "ការស្នើសុំនាំចូលរបស់លោកអ្នក បានរួចរាល់ សូមរង់ចាំ ការឆ្លើយតបពីក្រសួងបរិស្ថាន/ Your Request sent successful, Please wait for approval from Ministry of Environment.",
            );
    }
    ...
    public function showdetail_equipmentrequest($id)
    {
        ...
        return view(
            "showdetail_equipmentrequest",
            compact("Equipmentrequest", "Customer"),
        );
    }
    ...
    {
        $Material = Material::select("com_name", "chemical", "code")
            ->where("id", $request->id)
            ->first();
    ...
    public function iquota()
    {
        ...
        $Material = Material::select("substance", "id")
            ->where("status", 1)
            ->get();
    ...
    public function idata()
    {
        ...
        $isubstance = Materialrequest::join(
            "materialrequestdetails",
            "materialrequests.id",
            "=",
            "materialrequestdetails.materialrequest_id",
        )
            ->join(
                "customers",
                "materialrequests.customer_id",
            ...
            ->select(
                ...
                "company_name",
                "materialrequests.id",
                DB::raw("sum(materialrequestdetails.quantity) AS Total"),
                "customer_id",
                "import_status",
                "import_date",
                "materialrequests.created_at",
                "request_no",
            )
            ->groupBy(
                "materialrequests.id",
        ...
        $Customer = Customer::with("Cominfo")->find(Auth::id());
        DB::statement(DB::raw("set @rownum=0"));
        $request = Quotarequest::join(
            "customers",
            "quotarequests.customer_id",
            "=",
            "customers.id",
        )
            ->join(
                ...
                "quotarequests.id",
                "=",
                "quotarequestdetails.quotarequest_id",
            )
            ->select(
                DB::raw("@rownum  := @rownum  + 1 AS rownum"),
                "quotarequests.id",
                "quotarequests.created_at",
                DB::raw("sum(quotarequestdetails.amount) AS total"),
            )
        ...
        DB::statement(DB::raw("set @rownum=0"));
        $equipmentrequest = Equipmentrequest::join(
            "customers",
            "equipmentrequests.customer_id",
            "=",
            "customers.id",
            ...
            ->join(
                ...
                "equipmentrequests.id",
                "=",
                "equipmentrequestdetails.equipmentrequest_id",
            )
            ->select(
                DB::raw("@rownum  := @rownum  + 1 AS rownum"),
                "company_name",
                "equipmentrequests.id",
                DB::raw("sum(equipmentrequestdetails.amount) AS Total"),
                "customer_id",
                "import_status",
                "import_date",
                "equipmentrequests.created_at",
                "request_no",
            )
            ->groupBy(
                "equipmentrequests.id",
            ...
            ->where("customer_id", Auth::id())
            ->orderBy("id", "desc")
            ->paginate(10, ["*"], "publisheds");
        return view(
            "idata",
            compact("Customer", "request", "isubstance", "equipmentrequest"),
        );
    }
    ...
    {
        $Customer = Customer::find(Auth::id());
        $requestdetail = Quotarequest::with([
            "Quotarequestdetails.Material",
            "Quotarequestfiles",
            "Customer",
        ])->find($id);
        return view(
            "show_detail_request",
            compact("requestdetail", "Customer"),
        );
    }
    ...
    public function rquota(Request $request)
    {
        ...
        foreach ($request->input("amount") as $index => $value) {
            if ($value != null) {
                ...
                $all_data = [
                    ...
                    "material_id" => $request->material_id[$index],
                    "amount" => $value,
                    "old_amount" => empty($request->old_amount[$index])
                        ? 0
                        : $request->old_amount[$index],
                ];
        ...
        foreach ($users as $user) {
            if ($user->hasAnyPermission(21)) {
                Mail::send(
                    "emails.reminder",
                    ["requestdetail" => $questrequest, "user" => $user],
                    function ($m) use ($user) {
                        $m->from(
                            "developer@cems10.com",
                            "MOE Mail Auto System",
